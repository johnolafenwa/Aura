...
The condition must have type `bool`. Aurora does not apply Python-style
truthiness. The optional message must have type `String`.

## Evaluation Is Deliberately Lazy

Aurora evaluates the condition exactly once. When it is `true`, execution
...
Assertions are never removed by an optimization or release mode. Aurora has no
