...
The one-shot helpers and `fs.File` whole-file reads are capped at 256 MiB of remaining content in both `aura run` and built binaries. Aurora 0.1 has no incremental file-read member, so larger files need a host helper or pre-splitting.
