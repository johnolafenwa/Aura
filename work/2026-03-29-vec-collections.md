...
- Added `for value in vec:`, `for value in borrow vec:`, and `for value in borrow mut vec:` iteration support for `Vec[T]`.
- Fixed `for value in borrow mut vec:` so immutable vector bindings now fail during checking instead of being mutated through the loop body.
...
- Fixed follow-up checker/runtime parity gaps so `for value in borrow mut vec:` now rejects immutable vector places and `Vec[T]()` constructor locals keep their vector type through MIR/direct-backend `for` lowering.
