...
Bring the compiler and tooling into line with the Aurora v1 proposal by supporting ordinary function parameters written with `` and `mut `, instead of limiting borrowing syntax to method receivers.
...
- Updated semantic checking so:
  ...
  - `mut ` parameters require a mutable place at the call site
  - moving a non-copy field out of a borrowed ordinary parameter is rejected
  - borrowed parameters may not have default values
- Updated the interpreter so `mut ` parameters write caller-visible mutations back to the original argument place.
...
- The compiler still does not implement expression-level `` / `mut ` forms from the full proposal.
