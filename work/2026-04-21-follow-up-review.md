...
- Re-read the current checker and runtime changes in `sema.rs` and `runtime_value.rs`, with extra attention on the new `match mut` invalidation logic, module-qualified enum handling, and the reworked TLS accept loop.
- Reproduced a remaining `match mut` correctness hole where mutating the scrutinee through a `mut ` helper call still leaves later pattern-binding writes silently ineffective in both `aura run` and direct-built binaries.
...
- `./target/debug/aura check <temp repro>` and `./target/debug/aura run <temp repro>` for stale `match mut` bindings after `replace(x)`
...
- Extend stale `match mut` invalidation beyond explicit assignment statements so `mut ` calls and mutating receiver methods also retire old pattern bindings.
