...
| --- | --- | --- |
| 1 | Callable and type foundations | Add unions, aliases, owned-capture closure storage under ADR-0037, bound methods on owned or Copy receivers, and preserved names/defaults/keyword-only contracts. Defer stored ADR-0038 loan captures to the joint Batch 1–2 lifetime design. Structurally split `sema.rs` into type representation, capability checking, and place/loan analysis. Use the Option-removal criteria and intermediate checkpoint below. [Detailed design checkpoint](16-batch-1-design-checkpoint.md): Ratified; phase 1 records and implementation in progress on `codex/batch-1-phase-1`. Option library removal remains phase 2. |
| 1, after phase 2 (H2) | Ratified union/callable layout and allocation measurement | Implement Q9 A / Q15 A / Q16 A after Option removal, replacing the disclosed boxed union payload and `Arc` closure environment interim. Measure allocation behavior. No FFI/ABI stability claim and no 0.4 release before this phase is complete (owner ruling, 2026-09-13). |
| 2 | Natural collection access | [Batch 2–3 design checkpoint](17-batch-2-3-design-checkpoint.md): ratified 2026-09-20 by delegated decision; implementation follows the representation phase. Extend ADR-0038 to list elements, dictionary entries, and slice views. Make shared reads, explicit cloning, mutation, and ownership transfer compose naturally; define bounds, structural mutation, and invalidation rules. Revisit an app-facing `V \| None` form of `dict.get` with element loans, including result lifetime and present-None distinctions, as required by [Batch 1 Q23 ratification](16-batch-1-design-checkpoint.md#ratification-record). |
| 3 | Initialization and resource management | [Batch 2–3 design checkpoint](17-batch-2-3-design-checkpoint.md): ratified 2026-09-20 with Batch 2. Add `__init__(self, ...)`, definite initialization, named fallible factories, and generic/multiple typed context managers. Use one partial-construction cleanup mechanism extending ADR-0038's ordered exit-action stack for initializer failure, multi-resource entry failure, and failed decodes. Resolve cancellation/reset and header-temporary entry-view obligations below. |
