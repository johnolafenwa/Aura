...
| --- | --- | --- |
| 1 | Callable and type foundations | Add unions, aliases, owned-capture closure storage under ADR-0037, bound methods on owned or Copy receivers, and preserved names/defaults/keyword-only contracts. Defer stored ADR-0038 loan captures to the joint Batch 1–2 lifetime design. Structurally split `sema.rs` into type representation, capability checking, and place/loan analysis. Use the Option-removal criteria and intermediate checkpoint below. [Detailed design checkpoint](16-batch-1-design-checkpoint.md): Proposed; awaiting user ratification. |
