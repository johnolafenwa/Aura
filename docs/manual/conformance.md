...
| `enumerate`/`zip` loop-form recognition, operand domain, borrow default, `int64` positions, shortest-operand `zip` termination, shadowing, and backend parity | `enumerate_and_zip_iterate_in_lockstep_over_the_bare_loop_default`; `enumerate_requires_indexable_iterable` and `zip_rejects_ownership_modifiers` check-fail fixtures; the `enumerate_and_zip` run fixture and example; and the MIR/direct parity matrix |
