...
| dict duplicate-key replacement, key-before-value effects, indexed-read/simple-write ownership, and missing-key traps | `map_literal_duplicate_keys`, `map_index_non_copy_requires_explicit_clone`, `map_index_assignment_consumes_noncopy_key`, and `map_index_missing_key` fixtures plus the MIR/native parity matrix |
