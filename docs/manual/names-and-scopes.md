...
Associated methods are methods without `self` and are referenced through the type, for example `Worker.create(...)`. Instance methods require a receiver compatible with their declared value, shared-borrow, or mutable-borrow receiver.
