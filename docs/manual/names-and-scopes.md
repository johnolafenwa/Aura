...
Associated methods are methods without a receiver and are referenced through the type, for example `Worker.create(...)`. Instance methods require a receiver compatible with their declared shared (`self`/`borrow self`), consuming (`own self`), or mutable (`borrow mut self`) contract.
