...
`= ...` promises default availability. Bare destinations require identical
complete contracts (`AU2015`). An explicit thin alias call such as
`Unary(function)` may hide names, drop default availability, or restrict a
slot to keyword-only; inference never invents a common contract. Phase 6.2 uses that surface for
...
compiler-service/editor transport uses semantic schema version `16`. This
