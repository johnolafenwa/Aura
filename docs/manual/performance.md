...
`aba80f87` and archived with a checksum manifest in
...
reference numbers for its `Value` model, not an ABI claim. Closure
environments still allocate; the four-word callable layout (Q15 A / Q16 A)
is the phase's next pull request, and these rows are re-measured there.
