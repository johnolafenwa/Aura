...
The native path is content-addressed. A successful direct build publishes its binary into a cache keyed by this compiler's version, the host target, the backend, the linked runtime archive's content, and the complete lowered program, which already incorporates the entry source and every resolved dependency source. A later run with the same inputs launches the cached binary without compiling or linking. `AURORA_CACHE_DIR` selects the cache directory; the default is `~/.cache/aurora/native`. Entries are published atomically, so a concurrent run never launches a partially written executable, and a cache failure degrades to an ordinary build rather than to an error.

The default remains `mir`. A warm launch of a resident cached binary matches the MIR runtime, but a cold miss still costs about 1.3 seconds and a first touch of a fresh binary about 0.8 seconds, because a direct hello-world executable is roughly 57 MB of statically linked runtime. Workloads dominated by programs seen once, including CI, would pay that cost on every program. `aura build --backend direct` uses native direct emission, and `--backend auto` may select the checked MIR-launcher fallback. The language server delegates semantic analysis and completion to the persistent compiler service; its lexical fallback is recovery-only and is not a second language implementation.
