...
backend. A union whose members are all scalars, `None`, or plain classes is
one inline tagged value on the direct backend; a union with a string, list,
or other runtime-object member, and every closure environment, remain the
explicitly disclosed boxed interim until the second pull request of the
representation phase lands the ratified Q15 A / Q16 A callable layout; no
FFI/ABI stability claim and no 0.4 release precede that. A `TaskCallable[...]` value is a stored task
