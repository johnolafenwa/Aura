use aurora_compiler::ast::{ExternFunctionDecl, ExternOpaqueClassDecl, Item, TypeRefKind};
use aurora_compiler::ffi::{
    call_host_function, call_process_symbol, FfiError, FfiSignature, FfiType, FfiValue,
    HostFunction, OpaqueHandle,
};
use aurora_compiler::lexer::{lex, TokenKind};
use aurora_compiler::{analyze_path_source, analyze_source, complete_path_source, parse_source};
use std::ffi::c_void;
use std::fs;
...
fn opted_in_package_analysis_exposes_ffi_completions_hovers_and_definitions() {
    let temp_dir = TempDir::new("aurora-ffi-public-analysis");
    ...
    for (hover, line, start, end) in [
        ...
        assert_eq!(
            ...
            Some(&aurora_compiler::analysis::AnalysisRange {
...
fn process_symbol_api_resolves_system_functions_and_teaches_lookup_failures() {
    ...
    let missing =
        unsafe { call_process_symbol("__aurora_missing_public_ffi_symbol__", &signature, &mut []) }
            .expect_err("missing symbols should fail safely");
    assert!(matches!(
        &missing,
        FfiError::SymbolNotFound { symbol, detail }
            if symbol == "__aurora_missing_public_ffi_symbol__" && !detail.is_empty()
    ));
    assert!(missing
        .to_string()
        .starts_with("FFI symbol `__aurora_missing_public_ffi_symbol__` was not found:"));
...
fn public_path_check_preserves_canonical_imported_opaque_handle_identity() {
    let temp = TempDir::new("aurora-ffi-canonical-handle");
...
"#,
    ...
    let program = aurora_compiler::check_path(&main_path)
...
fn public_program_metadata_retrieves_the_checked_consuming_closure() {
    let temp = TempDir::new("aurora-ffi-closure-metadata");
    let main_path = write_checked_ffi_package(
        &temp,
        r#"
extern "C" opaque class Handle
...
"#,
    ...
    assert_eq!(
        retrieved.call_kind,
        aurora_compiler::sema::ClosureCallKind::Consuming
    );
...
fn public_from_import_preserves_extern_call_and_handle_identity_contracts() {
    let temp = TempDir::new("aurora-ffi-from-import-contract");
    let main_path = write_checked_ffi_package(
        &temp,
        r#"
...
fn public_module_exports_preserve_function_and_tuple_callable_contracts() {
    let temp = TempDir::new("aurora-exported-callable-contracts");
    let main_path = write_checked_ffi_package(
        &temp,
        r#"
import helpers
