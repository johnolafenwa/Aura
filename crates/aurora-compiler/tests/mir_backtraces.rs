use aurora_compiler::{run_path, run_source};
use std::fs;
...
fn nested_synchronous_trap_records_and_renders_the_aurora_call_chain_once() {
    ...
    assert_eq!(
        error
            .notes
            .iter()
            .filter(|note| note.starts_with("Aurora"))
            .count(),
        0
    );
    assert_eq!(
        error
            ...
            .collect::<Vec<_>>(),
        vec![
            ("explode", aurora_compiler::Span::new(1, 1)),
            ("relay", aurora_compiler::Span::new(5, 1)),
            ("main", aurora_compiler::Span::new(8, 1)),
        ]
    );
    assert!(error
        .call_frames
...
fn spawned_child_trap_records_and_renders_task_entry_and_spawn_ancestry() {
    ...
    assert!(error.notes.iter().all(|note| !note.starts_with("Aurora")));
...
fn nested_task_failure_keeps_youngest_first_typed_ancestry_exactly_once() {
    ...
    assert_eq!(
        error
            ...
            .collect::<Vec<_>>(),
        vec![
            ("leaf", "child", aurora_compiler::Span::new(7, 15)),
            ("child", "main", aurora_compiler::Span::new(12, 15)),
        ]
    );
    assert!(error.notes.iter().all(|note| !note.starts_with("Aurora")));
    ...
    assert_eq!(rendered.matches("Aurora task entry").count(), 1);
    assert_eq!(rendered.matches("Aurora task ancestry").count(), 1);
    assert!(rendered.contains(
        "Aurora task ancestry (youngest first): leaf spawned from child at 7:15 -> child spawned from main at 12:15"
    ));
}
