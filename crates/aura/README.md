...
- `aura lsp`
  ...
  - every request and response carries compiler-owned semantic-interface version `13`; a schema mismatch closes the connection before analysis
- `aura run [--backend mir|direct|auto] <file.au> [-- <program-args>...]`
  ...
  - cache keys independently include native cache format `v5`, semantic-interface schema `v13`, the exact linked runtime archive, and ordered native link arguments; inherited launch leases and owner-aware staging cleanup prevent interrupted-run cleanup from deleting a live native child
