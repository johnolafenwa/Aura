...
fn ffi_manifest_opt_in_defaults_off_and_allows_direct_root_use_when_enabled() {
    let temp = TempDir::new("aura-packages-ffi-root-opt-in");
...
fn extern_declarations_enforce_manifest_authorization_during_module_loading() {
    let temp = TempDir::new("aura-packages-ffi-module-authorization");
...
fn ffi_authorization_rejects_paths_outside_the_discovered_package_graph() {
    let temp = TempDir::new("aura-packages-ffi-unknown-source");
    let main_path = temp.write("app/src/main.au", "def main():\n    pass\n");
    temp.write(
        "app/Aura.toml",
        "[package]\nname = \"app\"\nversion = \"0.1.0\"\nedition = \"2026\"\nallow_ffi = true\n",
    ...
    assert_eq!(
        diagnostic.message,
        format!(
            "could not determine the Aura package for FFI declaration in `{}`",
...
fn ffi_dependency_report_accepts_listed_direct_and_transitive_dependencies() {
    let temp = TempDir::new("aura-packages-ffi-dependency-report");
...
fn ffi_dependency_report_names_the_missing_transitive_dependency_path() {
    let temp = TempDir::new("aura-packages-ffi-missing-report");
    let main_path = temp.write("app/src/main.au", "def main():\n    pass\n");
    temp.write(
        "app/Aura.toml",
        r#"[package]
...
fn ffi_dependency_report_requires_the_root_package_opt_in_too() {
    let temp = TempDir::new("aura-packages-ffi-root-dependency-opt-in");
    let main_path = temp.write("app/src/main.au", "def main():\n    pass\n");
    temp.write(
        "app/Aura.toml",
        r#"[package]
...
fn ffi_dependency_report_teaches_unopted_root_to_add_the_missing_report_entry() {
    let temp = TempDir::new("aura-packages-ffi-root-missing-report");
    let main_path = temp.write("app/src/main.au", "def main():\n    pass\n");
    temp.write(
        "app/Aura.toml",
        r#"[package]
...
fn ffi_dependency_missing_own_opt_in_names_its_dependency_path() {
    let temp = TempDir::new("aura-packages-ffi-dependency-own-opt-in");
    let main_path = temp.write(
        "app/src/main.au",
        "import native.sys\n\ndef main():\n    pass\n",
    );
...
fn ffi_dependency_report_rejects_unknown_and_non_ffi_entries() {
    let temp = TempDir::new("aura-packages-ffi-stale-report");
    let main_path = temp.write("app/src/main.au", "def main():\n    pass\n");
    temp.write("util/src/main.au", "def main():\n    pass\n");
    temp.write(
        "util/Aura.toml",
...
fn ffi_dependency_report_rejects_duplicate_invalid_and_self_entries() {
    let temp = TempDir::new("aura-packages-ffi-report-entry-validation");
    let main_path = temp.write("app/src/main.au", "def main():\n    pass\n");
    temp.write(
        "app/Aura.toml",
        r#"[package]
...
fn init_git_package_repo(temp: &TempDir, relative: &str, package_name: &str) -> PathBuf {
    ...
    run_git(&repo, &["config", "user.name", "Aura Tests"]);
    run_git(&repo, &["config", "user.email", "aura-tests@example.com"]);
    run_git(&repo, &["add", "."]);
...
fn reject_symlinks_in_tree_reports_symlinked_entries() {
    let temp = TempDir::new("aura-package-symlink-tree");
...
fn read_cached_git_revision_rejects_symlinked_markers() {
    let temp = TempDir::new("aura-package-symlink-marker");
    let root = temp.path.join("checkout");
    ...
    std::os::unix::fs::symlink(&target, root.join(".aura-cache-rev"))
...
fn package_manifest_and_lockfile_helpers_report_current_diagnostics() {
    let temp = TempDir::new("aura-package-manifest-lockfile-helpers");

    temp.write("missing_package/Aura.toml", "[dependencies]\n");
    assert!(expect_diag(
    ...
    for (name, source, expected) in manifest_cases {
        temp.write(&format!("{}/Aura.toml", name), source);
    ...
    temp.write("parse_error/Aura.toml", "[package");
    assert!(expect_diag(
        load_raw_manifest(&temp.path.join("parse_error/Aura.toml")),
    ...
    for (name, source, expected) in lockfile_cases {
        temp.write(&format!("{}/Aura.lock", name), source);
...
fn package_update_and_workspace_helpers_cover_edge_paths() {
    let temp = TempDir::new("aura-package-update-helpers");
    temp.write(
        "app/Aura.toml",
        "[package]\nname = \"app\"\nversion = \"0.1.0\"\nedition = \"2026\"\n\n[dependencies]\nutil = { path = \"../util\" }\n",
    );
    ...
    temp.write("empty_workspace/Aura.toml", "[workspace]\nmembers = []\n");
    let empty_workspace =
    ...
    temp.write(
        "workspace/app/Aura.toml",
        "[package]\nname = \"app\"\nversion = \"0.1.0\"\nedition = \"2026\"\n",
    );
    temp.write("workspace/app/src/main.au", "def main():\n    pass\n");
    temp.write(
        "workspace/lib/Aura.toml",
        "[package]\nname = \"lib\"\nversion = \"0.1.0\"\nedition = \"2026\"\n",
    );
    ...
    let lockfile = fs::read_to_string(temp.path.join("workspace/Aura.lock"))
...
fn package_resolver_reports_graph_limit_and_workspace_lookup_edges() {
    let temp = TempDir::new("aura-package-graph-limits");
    ...
    temp.write("workspace_only/Aura.toml", "[workspace]\nmembers = []\n");
...
fn package_lookup_helpers_propagate_manifest_parse_errors() {
    let temp = TempDir::new("aura-package-lookup-parse-errors");

    temp.write("bad_package/Aura.toml", "[package");
    ...
    temp.write("bad_workspace/Aura.toml", "[workspace");
    temp.write(
        "bad_workspace/app/Aura.toml",
        "[package]\nname = \"app\"\nversion = \"0.1.0\"\nedition = \"2026\"\n",
    );
...
fn package_resolver_reports_missing_source_roots_and_dependency_paths() {
    let temp = TempDir::new("aura-package-missing-source-roots");
...
fn package_workspace_update_marks_non_member_dependencies_external() {
    let temp = TempDir::new("aura-package-workspace-external-deps");
    temp.write("workspace/Aura.toml", "[workspace]\nmembers = [\"app\"]\n");
    write_package(
        &temp,
        "workspace/app",
        "app",
...
fn package_path_cache_and_validation_helpers_cover_remaining_edges() {
    let temp = TempDir::new("aura-package-path-cache-helpers");
...
fn package_graph_helpers_report_unusual_but_supported_edges() {
    let temp = TempDir::new("aura-package-graph-helper-edges");
    assert!(
    ...
    let outside_entry = PackageGraph::discover_for_entry(&app_dir.join("Aura.toml"))
    ...
    let lockfile = fs::read_to_string(app_dir.join("Aura.lock"))
...
#[test]
fn package_discovery_accepts_only_the_aura_manifest_name() {
    let temp = TempDir::new("aura-package-name-contract");
    let old_manifest = ["Auro", "ra.toml"].concat();
    temp.write(
        &format!("old/{old_manifest}"),
        "[package]\nname = \"old\"\nversion = \"0.1.0\"\nedition = \"2026\"\n",
    );
    temp.write("old/src/main.au", "def main():\n    print(1)\n");
    let old_entry = fs::canonicalize(temp.path.join("old/src/main.au"))
        .expect("old-name fixture entry should canonicalize");
    assert!(
        PackageGraph::discover_for_entry(&old_entry)
            .expect("an unrecognized manifest name should not break loose-source discovery")
            .is_none(),
        "the old manifest filename must not create a package"
    );
    ...
    temp.write("current/src/main.au", "def main():\n    print(2)\n");
    let current_entry = fs::canonicalize(temp.path.join("current/src/main.au"))
        .expect("Aura fixture entry should canonicalize");
    let graph = PackageGraph::discover_for_entry(&current_entry)
        .expect("Aura manifest discovery should succeed")
        .expect("Aura.toml should create a package");
    assert_eq!(
        graph.root_source_root,
        fs::canonicalize(temp.path.join("current/src"))
            .expect("Aura source root should canonicalize")
    );
}

#[test]
fn package_resolver_reports_cycle_duplicate_and_expected_name_edges() {
    let temp = TempDir::new("aura-package-resolver-edges");
    write_package(
        &temp,
        "cycle_app",
        "cycle_app",
...
fn package_io_helpers_cover_local_error_paths() {
    ...
    fs::create_dir_all(temp.path.join("lockdir/Aura.lock"))
    ...
    assert!(normalize_relative_path(Path::new("/tmp/aura")).starts_with('/'));

    let original_timeout = std::env::var_os("AURA_GIT_TIMEOUT_MS");
    std::env::set_var("AURA_GIT_TIMEOUT_MS", "25");
    assert_eq!(git_command_timeout(), StdDuration::from_millis(25));
    std::env::set_var("AURA_GIT_TIMEOUT_MS", "0");
    assert_eq!(git_command_timeout(), DEFAULT_GIT_COMMAND_TIMEOUT);
    std::env::set_var("AURA_GIT_TIMEOUT_MS", "not-a-number");
    assert_eq!(git_command_timeout(), DEFAULT_GIT_COMMAND_TIMEOUT);
    match original_timeout {
        Some(value) => std::env::set_var("AURA_GIT_TIMEOUT_MS", value),
        None => std::env::remove_var("AURA_GIT_TIMEOUT_MS"),
    }
    ...
    assert_eq!(
        git_cache_root(),
        std::env::temp_dir().join("aura").join("git")
    );
    ...
    assert_eq!(
        git_cache_roots(),
        vec![
            distinct_cache_home.join("aura").join("git"),
            std::env::temp_dir().join("aura").join("git")
        ]
    );
    match original_cache_home {
    ...
    let cache_home_file = temp.write("cache-home-file", "not a directory");
    {
        ...
        let cache_error = ensure_git_checkout("https://example.com/aura-test.git", "abcdef0")
    ...
    let failure = run_git_command(None, vec!["not-a-real-aura-test-command".to_string()])
        .expect_err("invalid git subcommands should report status failure");
    assert!(failure.message.contains("git not-a-real-aura-test-command"));
    ...
    #[cfg(unix)]
    {
        ...
        let spawn_error = run_command_with_timeout(
            Command::new("__definitely_missing_aura_test_command__"),
            "missing-command",
        ...
        let stdout_failure = run_git_command(
            ...
            vec![
                "-c".to_string(),
                "alias.aura-stdout-fail=!f() { printf stdout-only; exit 7; }; f".to_string(),
                "aura-stdout-fail".to_string(),
            ],
        )
...
fn package_unix_cache_helpers_reject_symlinked_manifest_and_missing_dirs() {
    let temp = TempDir::new("aura-package-unix-cache-edges");
    ...
    fs::write(marker_checkout.join(".aura-cache-rev"), "abcdef0\n")
    ...
    fs::create_dir_all(marker_dir_checkout.join(".aura-cache-rev"))
