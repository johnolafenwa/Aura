...
const MANIFEST_NAME: &str = "Aura.toml";
const LOCKFILE_NAME: &str = "Aura.lock";
...
fn resolve_package_graph_for_update(
    ...
    let Some(workspace_root) = find_enclosing_workspace_root_from_dir(start_dir)? else {
        return Err(Diagnostic::new(format!(
            "could not find an enclosing Aura package or workspace starting from `{}`",
            start_dir.display()
        )));
    };
...
fn load_package_manifest(manifest_dir: &Path) -> Result<ParsedPackageManifest> {
    ...
    if package.edition != SUPPORTED_PACKAGE_EDITION {
        return Err(Diagnostic::new(format!(
            "manifest `{}` uses unsupported package edition `{}`; the current Aura compiler supports edition `{}`",
            manifest_path.display(),
            package.edition,
...
fn git_cache_roots() -> Vec<PathBuf> {
    ...
    let fallback = env::temp_dir().join("aura").join("git");
    if primary == fallback {
        vec![primary]
    } else {
...
fn run_command_with_timeout(
    ...
    #[cfg(unix)]
    {
        ...
        // Give the child a fresh process group. Timeout cleanup can then kill
    ...
    let failure = loop {
        match child.try_wait() {
            ...
            Ok(None) => {}
            Err(error) => {
                break Diagnostic::new(format!("failed to wait for `{}`: {}", display_name, error));
            }
        }
        if started.elapsed() >= timeout {
            break Diagnostic::new(format!("`{}` timed out after {:?}", display_name, timeout));
        }
...
fn git_cache_root() -> PathBuf {
    if let Some(cache_home) = env::var_os("XDG_CACHE_HOME") {
        return PathBuf::from(cache_home).join("aura").join("git");
    }
    if let Some(home) = env::var_os("HOME") {
        return PathBuf::from(home).join(".cache").join("aura").join("git");
    }
    env::temp_dir().join("aura").join("git")
}
...
fn cached_git_revision_path(checkout_dir: &Path) -> PathBuf {
    checkout_dir.join(".aura-cache-rev")
}
...
fn read_cached_git_revision(checkout_dir: &Path) -> Result<Option<String>> {
    ...
    let Some(mut marker) = open_nofollow_file_at(&dir_fd, ".aura-cache-rev", checkout_dir)? else {
        return Ok(None);
    };
