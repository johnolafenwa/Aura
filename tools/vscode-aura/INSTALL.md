...
This guide covers both development use from this monorepo and installing Aura as a packaged VS Code extension.
...
The Aura extension will activate automatically for `.au` files and start the bundled Aura language server.
...
actual compiler-owned server on `PATH` for all Aura workspaces, run:
...
`tools/aura-language-server/src/server.js` and writes the self-contained
...
4. Select:
   - `tools/vscode-aura/aura-language.vsix`
...
When the Aura language server or extension changes, run
...
The Aura VS Code tooling currently provides:
...
The LSP implementation currently comes from `tools/aura-language-server`, and the extension bundles that server into its own `dist/` output during build.
