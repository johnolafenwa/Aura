...
This extension lives in the Aura monorepo so editor tooling evolves alongside the language.
...
- Aura-specific Enter handling so block indentation behaves predictably in off-side code
- snippets for common Aura constructs
- LSP client for the Aura language server
...
The language intelligence comes from the in-repo `aura-language-server`
...
The VS Code extension now bundles its client and server entrypoints into `tools/vscode-aura/dist/`, so packaged VSIX installs do not depend on sibling files outside the extension folder.
...
That will start the Aura language server automatically and enable syntax highlighting, completions, hover, go-to-definition, diagnostics, and document symbols.
...
`aura-language.vsix` is an ignored local build artifact, so a file left from
...
`tools/vscode-aura/aura-language.vsix`, and reload when prompted.
...
For a step-by-step guide inside the repo, see `tools/vscode-aura/INSTALL.md`.
...
As the compiler grows, this extension should stay a thin client while the Aura language server becomes the canonical place for editor intelligence.
